...
import { LayoutDashboard, MapPin, ClipboardList, Route, FileText, CalendarDays, Menu, X } from "lucide-react";
import { useState, useEffect } from "react";

const navItems = [
  { href: "/", label: "Dashboard", icon: LayoutDashboard, short: "Home" },
  { href: "/standorte", label: "Standorte", icon: MapPin, short: "Standorte" },
  { href: "/auftraege", label: "Aufträge", icon: ClipboardList, short: "Aufträge" },
  { href: "/offerten", label: "Offerten", icon: FileText, short: "Offerten" },
  { href: "/wochenplan", label: "Wochenplan", icon: CalendarDays, short: "Woche" },
  { href: "/routen", label: "Routenplanung", icon: Route, short: "Routen" },
];
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-64 bg-[var(--color-primary)] text-white flex-col z-50">
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`flex items-center gap-3 px-5 py-3 text-sm transition-colors ${
                  isActive ? "bg-white/15 text-[var(--color-accent)] border-r-3 border-[var(--color-accent)]"
        ...
        <div className="p-4 border-t border-white/10 text-xs text-white/40">© 2026 Galvomag AG<br />Regensdorf</div>
      </aside>
                ...
                return (
                  <Link key={item.href} href={item.href}
                    className={`flex items-center gap-3 px-5 py-3.5 text-sm transition-colors ${
                      isActive ? "bg-white/15 text-[var(--color-accent)]" : "text-white/80 hover:bg-white/8"
                    }`}>
